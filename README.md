...
Synthetic speech clones the stock network-player voice source's mixer and spatialization route when that source becomes available. Big Walk's direct-voice attenuation curve is keyed in world metres, so Ramblers evaluates it directly against human-to-companion distance; it does not install that curve as a Unity custom rolloff, which would rescale the curve over `AudioSource.maxDistance` and change the falloff. Route selection, reference curve samples, live distance, and applied output level are logged for runtime comparison.
...
| `interact_with_object(target: switch ID \| human_reference \| companion_held_item)` | Walks to and performs one primary click on an exact nearby switch, the exact world switch indicated by the human, or the exact prop already in its hands. |
...
Each human utterance also receives a bounded nonverbal game-context packet. It reports current human/companion relationship and action state, up to six nearby props, six active primary-interaction switches, and three other players with stable IDs and coarse spatial/interaction facts, and only the significant events not already reported from an eight-entry journal. Rambler's existing natural ambient glances can populate one local visual-memory slot after the gaze visibly settles; novelty, a 30-second capture interval, 45-second freshness and one-shot delivery prevent that from becoming continuous surveillance or an accumulating local photo stream. Context is consumed only after it is successfully queued and never creates a response by itself, so awareness improves ordinary conversation without making the companion narrate every scene.

`interact_with_object` freezes both kinds of Big Walk primary interaction available at the utterance boundary: an active world `CastableTarget`, including one selected from bounded nearby context without direct pointing, and the `useHeldSwitch` on the exact prop already in Rambler's hands. The model silently resolves natural wording such as “press the red-light button,” “operate that light control,” or “use it” after a pickup. For a world target, Rambler walks toward the exact switch with body-clearance and forward-ground checks, visibly aims, revalidates the same object plus companion-specific conditions, reach, blockers, and host authority, then uses the server-owned switch-state path and waits for that exact state to change. This is one primary interaction, not an idempotent named-state setter: it does not claim a final on/off state that was not actually observed. It never invokes a client command or searches for a replacement. Live compatibility with the red-light button and other puzzle switches remains a runtime QA question.
